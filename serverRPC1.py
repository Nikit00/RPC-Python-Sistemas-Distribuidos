...
with SimpleXMLRPCServer(('192.168.0.121', 9000), # Definindo o IP da maquina servidor e a porta de conexao
		requestHandler=RequestHandler) as server:
	server.register_introspection_functions()

	def buscar_carro(nome):
			
		con = mysql.connector.connect(host='localhost',database='VOLKS',user='root',password='102030hh')
		if con.is_connected():
